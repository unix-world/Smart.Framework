<?php
// [@[#[!NO-STRIP!]#]@]
// [AppCodeUnpack / INIT] v.20220730.1558 s.20220419.2004
...
//const SMART_FRAMEWORK_SRVPROXY_CLIENT_IP =  			'HTTP_X_FORWARDED_FOR'; 					// used to detect the real client's IP behind a load balancer / reverse proxy (ex: haproxy or varnish) ; here must be set the trusted header key that returns the real client IP (ex: use a trusted header like 'HTTP_X_FORWARDED_FOR' or 'HTTP_X_REAL_IP' or 'HTTP_CLIENT_IP' that is considered the trusted real visitor's IP header instead of the default 'REMOTE_ADDR' which in this case may become the proxy's IP address instead of clien's IP address) ; if no proxy server is set this must NOT be defined at all as the default TRUSTED key is always 'REMOTE_ADDR'
//const SMART_FRAMEWORK_SRVPROXY_CLIENT_PROXY_IP =  	'<HTTP_CLIENT_IP>,<HTTP_X_REAL_IP>'; 		// used to detect the real client's Proxy IP behind a load balancer / reverse proxy (ex: haproxy or varnish) ; can use one or many keys like 'HTTP_X_FORWARDED_FOR' or 'HTTP_X_REAL_IP' or 'HTTP_CLIENT_IP' but not the same value as set above at SMART_FRAMEWORK_SRVPROXY_CLIENT_IP ...
...
const SMART_FRAMEWORK_IDENT_ROBOTS =  					'<(smart.framework>,<(smart.framework.go>,<robot>,<apache>,<httperf>,<benchmark>,<scanner>,<Googlebot>,<AdsBot-Google>,<APIs-Google>,<Mediapartners-Google>,<FeedFetcher-Google>,<Google-Read-Aloud>,<DuplexWeb-Google>,<Storebot-Google>,<googleweblight>,<Yahoo! Slurp>,<slurp;>,<(slurp)>,<(slurp/>,<bingbot/>,</bingbot.>,<bing bot>,<msnbot>,<ms url control>,<winhttp>,<YandexBot>,<yandex.com/bots>,<Baiduspider>,<DuckDuckBot>,<exabot>,<Jeeves/>,<ia_archiver>,<facebot>,<facebookexternalhit/>,<LinkedInBot/>,<wget>,<curl/>,<Go-http-client/>,<php>,<php/>,<libwww-perl>,<python-urllib>,<java>,<htmldoc>,<webcrawler>,<domaincrawler>,<catchbot>,<webalta crawler>,<superbot>,<roku dvp>,<linkwalker>,<aihitbot>,<sanszbot>,<linguee bot>,<swish-e>,<tarantula>,<fast-webcrawler>,<teoma>,<everyfeed spider>,<gregarius>,<email wolf>,<gaisbot>,<gulperbot>,<grub-client>,<peach>,<htmlparser>,<w3c css validator>,<w3c (x)html validator>,<w3c p3p validator>,<download demon>,<offline explorer>,<webcopier>,<webzip>,<web downloader>'; // r.20220419 ; case insensitive ; spaces will be preserved ; can be array or string list ; example: '<bot signature 1>,<bot signature 2 >,< another-bot >' OR [ 'bot signature 1', 'bot signature 2 ', ' another-bot ' ]
