 ...
 *
 * includes fixes by unixman, r.20220904:
 * 	- XHTML tags fix to HTML5 to work with newer jQuery versions
 * 	- <var></var> changed to <dfn></dfn> to avoid conflicts with markdown
 *
 */
...
function getInterface(v) {
	...
	var AbstractMathQuill = APIClasses.AbstractMathQuill = P(Progenote, function(_) {
		...
		_.__mathquillify = function(classNames) {
			...
			root.jQ =
				$('<span class="mq-root-block"></span>').attr(mqBlockId, root.id).appendTo(el);
...
Controller.open(function(_) {
	Options.p.substituteTextarea = function() {
		return $('<textarea autocapitalize=off autocomplete=off autocorrect=off ' +
							 'spellcheck=false x-palm-disable-ste-all=true></textarea>')[0];
	};
...
var Variable = P(Symbol, function(_, super_) {
	_.init = function(ch, html) {
		super_.init.call(this, ch, '<dfn>'+(html || ch)+'</dfn>');
	};
	_.text = function() {
...
LatexCmds.f = P(Letter, function(_, super_) {
	_.init = function() {
		Symbol.p.init.call(this, this.letter = 'f', '<dfn class="mq-f">f</dfn>');
	};
	_.italicize = function(bool) {
...
LatexCmds.Upsih = //'cos it makes sense to me
	bind(Symbol,'\\Upsilon ','<dfn style="font-family: serif">&upsih;</dfn>'); //Symbola's 'upsilon with a hook' is a capital Y without hooks :(
...
var SupSub = P(MathCommand, function(_, super_) {
	...
	_.addBlock = function(block) {
		if (this.supsub === 'sub') {
			...
			block.jQ = $('<span class="mq-sup"></span>').append(block.jQ.children())
