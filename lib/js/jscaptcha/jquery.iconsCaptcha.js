	...
	$.fn.extend({
		...
		//	};
			...
			const initCaptcha = (arr, $id, $container) => {
				...
				if(!i2) {
					_p$.error('iCaptcha Unique Selection Failed:', $id);
					return;
				}
				...
				$container.find('div.iCaptcha-icon').each((ix, elm) => {
					...
					if(icon) {
						$(elm).attr('data-icaptcha-idx', ix).data('icaptcha-icn', icon).addClass(String(icon) + ' sfi-2x');
					}
				});
			...
			setTimeout(() => {
				...
				$.get($opts.iconsURL, (txt) => {

					if(!txt) {
						_p$.error('iCaptcha: FAILED to initialize the icons list');
						return;
					}
					...
					txt = null;
					if(arr.length <= 0) {
						_p$.error('iCaptcha: FAILED to process the icons list');
						return;
					}
					...
					$fX = _Y$.each((idx, el) => {
						...
						$container.on('click', '.iCaptcha-line > .iCaptcha-icon', (evt) => {
							...
							let _x = (evt.pageX - $(evt.target).offset().left);
							let _y = (evt.pageY - $(evt.target).offset().top);
							if(!_x || !_y) {
								return; // detect click coordinates ; if not pass here it is not a real click event, it is emulated, stop here
							}
