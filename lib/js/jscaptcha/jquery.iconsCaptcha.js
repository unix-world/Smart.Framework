			...
			setTimeout(() => {
				...
				$.get($opts.iconsURL, (txt) => {
					...
					$fX = _Y$.each((idx, el) => {
						...
						$container.on('click mousedown touchstart', '.iCaptcha-line > .iCaptcha-icon', (evt) => {
							...
							let _x = ((pointEv.pageX || 0) - $selected.offset().left);
							let _y = ((pointEv.pageY || 0) - $selected.offset().top);
