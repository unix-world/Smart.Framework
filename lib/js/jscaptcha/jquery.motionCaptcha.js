...
/**
 * jQuery Smart MotionCAPTCHA r.20210507
			...
			var touchEndEvent = function(event) {
				...
				if (!locked && _isDown) {
					...
					if (_points.length >= 10) {
						...
						if ($canvas.attr('class').match(result.Name) && result.Score >= Number(opts.similarity)) { // OK
