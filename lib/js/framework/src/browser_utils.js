...
const smartJ$Browser = new class{constructor(){ // STATIC CLASS
	 ...
	 */
	const OverlayShow = function(text, title, css_class, overlay_id=null) { // ES6
		//--
	 ...
	 * @param 	{JS-Code} 	evcode 					*Optional* the JS Code to execute on SUCCESS answer (before anything else)
	 * @param 	{JS-Code} 	everrcode 				*Optional* the JS Code to execute on ERROR answer (before anything else)
	 * @param 	{JS-Code} 	evfailcode 				*Optional* the JS Code to execute on REQUEST FAIL answer
	...
	const SubmitFormByAjax = function(the_form_id, url, growl='auto', evcode=null, everrcode=null, evfailcode=null, failalertable=false) { // ES6
		...
		ajax.done((msg) => { // {{{JQUERY-AJAX}}}
			...
			//--
			AlertDialog('<h4>HTTP&nbsp;Status: ' + _Utils$.escape_html(msg.status) + _Utils$.escape_html(HTTP_STATUS_CODES[String(msg.status)] ? ' ' + HTTP_STATUS_CODES[String(msg.status)] : 'Unknown') + '</h4><br>' + '\n' + '<h5>Invalid XHR Server Response ...</h5>', evfailcode, 'FAIL', null, null, dlg); // + '\n' + msg.responseText (but this is not safe to display)
			OverlayHide();
			//--
		});
		//--
	...
	const LoadElementContentByAjax = function(y_elemID, y_img_loader, y_url, y_method, y_data_type, y_data_arr_or_serialized, y_replace=false, y_notifyerr=null) {
		...
		ajax.done((msg) => { // {{{JQUERY-AJAX}}}
			...
			if(notifyErr === true) {
				GrowlNotificationAdd('HTTP&nbsp;Status: ' + _Utils$.escape_html(msg.status) + _Utils$.escape_html(HTTP_STATUS_CODES[String(msg.status)] ? ' ' + HTTP_STATUS_CODES[String(msg.status)] : 'Unknown'), 'FAIL: Invalid Server Response for: ' + _Utils$.escape_html(_m$) + '<br>' + '<b><i>Details:</i></b><br>' + _Utils$.nl2br(_Utils$.escape_html(_Utils$.stringReplaceAll(' ; ', '\n', errDetails))), _C$.param_ImgNotOK, 0, true, 'dark'); // , msg.responseText
			} else {
