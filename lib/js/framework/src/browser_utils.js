...
var SmartJS_BrowserUtils = new function() { // START CLASS
	 ...
	 * @param	{Numeric} 	expire 			*Optional* The cookie expire time in seconds since unix epoch (or set it to FALSE or zero to expire by session) ; (default is 0)
	...
	this.setCookie = function(name, value, expire, path, domain, samesite, secure) {
		...
		if(expire) {
			...
			if((SmartJS_CoreUtils.isFiniteNumber(expire)) && (expire > 0)) {
				d.setTime(d.getTime() + (expire * 1000)); // now + (seconds * 1000)
			} else {
				expire = 0;
			} //end if else
		} else {
	...
	this.deleteCookie = function(name, path, domain, samesite, secure) {
		//--
		_class.setCookie(name, '', 0, path, domain, samesite, secure);
		//--
	} //END FUNCTION
