...
var SmartJS_BrowserUtils = new function() { // START CLASS
	...
	var GrowlNotificationDoAdd = function(title, text, image, time, sticky, class_name, options) {
		...
		if(theGrowlType === 'gritter') {
			//--
			class_name = String(jQuery.gritter.translateCssClasses('toastr', class_name));
			//--
			try {
				jQuery.gritter.add({
					class_name: String(class_name),
			...
			//--
			var notifType = jQuery.toastr.translateCssClasses('gritter', class_name);
			//--
			try {
				jQuery.toastr[String(notifType)](
					String(text),
					String(String(title) + String(image)),
					{
						newestOnTop: true, // inverse against gritter, to make a difference if selected
						tapToDismiss: false,
						closeButton: true,
						progressBar: true,
