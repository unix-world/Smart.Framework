...
// DEPENDS: jQuery, smartJ$Utils, smartJ$CryptoHash, smartJ$Base64
 ...
 * @requires		*smartJ$UI
 *
 * @desc The JavaScript class provides methods to simplify the interraction with the Browser, Ajax, Forms and provide basic implementation of several UI and Util components.
...
const smartJ$Browser = new class{constructor(){ // STATIC CLASS
	...
	const _Option$ = ((typeof(smartJ$Options) != 'undefined') && smartJ$Options && $.isPlainObject(smartJ$Options) && (smartJ$Options.BrowserUtils != undefined) && $.isPlainObject(smartJ$Options.BrowserUtils)) ? smartJ$Options.BrowserUtils : null;
	 ...
	 * @memberof smartJ$Browser
	 */
	_C$.param_NotificationTimeOK = (_Option$ && (typeof(_Option$.NotificationTimeOK) == 'number') && _Option$.NotificationTimeOK && _Utils$.isFiniteNumber(_Option$.NotificationTimeOK)) ? _Utils$.format_number_int(_Option$.NotificationTimeOK, false) : 1500;
	 ...
	 * If set to FALSE will enable the modal iframe cascading (very inefficient) ; otherwise will use PopUp every next level starting from the ModalBox iframe level, but inside PopUp can open another ModalBox ... and so on
	 * Allowed Values: true | false
	 * @default true
	 ...
	 * @memberof smartJ$Browser
	 */
	_C$.param_LoaderBlank = (_Option$ && (typeof(_Option$.LoaderBlank) == 'string') && _Option$.LoaderBlank) ? _Utils$.stringTrim(_Option$.LoaderBlank) : 'lib/js/framework/blank.html';
	 ...
	 * @memberof smartJ$Browser
	 */
	_C$.param_ImgOK = (_Option$ && (typeof(_Option$.ImgOK) == 'string') && _Option$.ImgOK) ? _Utils$.stringTrim(_Option$.ImgOK) : 'lib/framework/img/sign-info.svg';
	 ...
	 * @memberof smartJ$Browser
	 */
	_C$.param_CssOverlayBgColor = (_Option$ && (typeof(_Option$.CssOverlayBgColor) == 'string') && _Option$.CssOverlayBgColor && String(_Option$.CssOverlayBgColor).match(/^\#([0-9a-f]{6})$/i)) ? _Utils$.stringTrim(_Option$.CssOverlayBgColor) : '#777777';
	...
	/**
	 * The time in microseconds for delayed page redirects
	 * @default 1500
	 * @var {Integer} param_TimeDelayRedirect
	 * @set [before] smartJ$Options.BrowserUtils.TimeDelayRedirect
	 * @static
	 * @private
	 * @memberof smartJ$Browser
	 */
	_C$.param_TimeDelayRedirect = (_Option$ && (typeof(_Option$.TimeDelayRedirect) == 'number') && _Option$.TimeDelayRedirect && _Utils$.isFiniteNumber(_Option$.TimeDelayRedirect)) ? _Utils$.format_number_int(_Option$.TimeDelayRedirect, false) : 1500;
	...
	const evFXEnd = ' })();';
	//--
	const cssAlertable = 'background:#ECECEC; font-size:1.5rem; font-weight:bold; text-align:right; padding:3px; padding-left:10px; padding-right:10px;';
	//--
	 ...
	 */
	const CountDown = function(counter, elID, evcode) { // ES6
		//--
		const _m$ = 'CountDown';
		//--
			...
			if(counter > 0) {
				...
				if((evcode != undefined) && (evcode != 'undefined') && (evcode != '')) { // undef tests also for null
					try {
						...
						_p$.error(_N$, _m$, 'ERR: JS-Eval Error');
					} //end try catch
				} //end if
				//--
			} //end if
	 ...
	 */
	const OverlayShow = function(text, title, css_class) { // ES6
		//--
		...
		const overlay = $('<div id="smart_framework_overlay" ' + the_style + '></div>').css({ 'z-index': 2147400000, 'position': 'fixed', 'top': '0px', 'left': '0px', 'width': '100%', 'height': '100%' }).hide().appendTo('body');
		$('#smart_framework_overlay').html('<div style="width:100%; position:fixed; top:25px; left:0px;"><center><div>' + inner_html + '</div></center></div>');
		//--
		try {
			overlay.fadeIn();
		} catch(err){
	 ...
	 * @arrow
	 */
	const OverlayClear = () => { // ES6
		//--
		$('#smart_framework_overlay').empty().html('');
		//--
	}; //END
	 ...
	 * @static
	 */
	const OverlayHide = function() { // ES6
		//--
		let overlay = $('#smart_framework_overlay');
		//--
		try {
			overlay.fadeOut();
		} catch(err){
	 ...
	 */
	const AlertDialog = function(y_message, evcode=null, y_title='', y_width=null, y_height=null) {
		//--
		...
		//--
		const dialogType = DialogSelectType();
		//--
		if(dialogType === 'ui') { // use UI Dialog (the best choice)
			...
			if(dialogType === 'alertable') {
				//--
				$.alertable.alert((y_title ? '<div style="' + _Utils$.escape_html(cssAlertable) + '">' + y_title + '</div>' : '') + y_message, { html:true }).always(() => { // use always not done to simulate real alert
	 ...
	 */
	const ConfirmDialog = function(y_question, evcode=null, y_title='', y_width=null, y_height=null) { // ES6
		//--
		...
		if(dialogType === 'ui') { // use UI Dialog (the best choice)
			...
			if(dialogType === 'alertable') {
				//--
				$.alertable.confirm((y_title ? '<div style="' + _Utils$.escape_html(cssAlertable) + '">' + y_title + '</div>' : '') + y_question, { html:true }).then(() => {
					if((evcode != undefined) && (evcode != '') && (evcode != 'undefined')) { // test undefined is also for null
						try {
							if(typeof(evcode) === 'function') {
	...
	const MessageNotification = function(ytitle, ymessage, yredirect, growl, class_growl, timeout) { // ES6
		...
		if(growl === 'yes') {
			...
			if((yredirect != undefined) && (yredirect != '')) { // undef tests also for null
				RedirectDelayedToURL(yredirect, (timeout + _Utils$.format_number_int(_C$.param_TimeDelayRedirect, false))); // comment it to disable override of mouse over growl
				redirectAfterClose = () => {
					RedirectDelayedToURL(String(yredirect), 500);
				};
			} else {
				setTimeout(() => { OverlayHide(); }, (timeout + _Utils$.format_number_int(_C$.param_TimeDelayRedirect, false))); // comment it to disable override of mouse over growl
			} //end if
			//--
			const growlOptions = {
	...
	const PopUpSendForm = function(objForm, strTarget, windowWidth=0, windowHeight=0, forcePopUp=0, forceDims=0, evcode=null) { // ES6
		...
		//--
		setTimeout(fx, 500); // delay submit for buggy browsers
		//--
		return false;
		//--
	}; //END
	 ...
	 */
	const PopUpLink = function(strUrl, strTarget, windowWidth=0, windowHeight=0, forcePopUp=0, forceDims=0) { // ES6
		//--
		...
		//--
		initModalOrPopUp(strUrl, strTarget, windowWidth, windowHeight, forcePopUp, forceDims);
		//--
		return false;
		//--
	}; //END
	 ...
	 * @hint The input needs the html attribute: onKeyDown="smartJ$Browser.catchKeyENTER(event);"
	 *
	 * @memberof smartJ$Browser
	 ...
	 * @hint The input needs the html attribute: onKeyDown="smartJ$Browser.catchKeyTAB(event);"
	 *
	 * @memberof smartJ$Browser
	 * @method catchKeyTAB
	 * @static
	 ...
	 */
	const SubmitFormByAjax = function(the_form_id, url, growl='auto', evcode=null, everrcode=null) { // ES6
		//--
		...
		ajax.done((msg) => { // {{{JQUERY-AJAX}}}
			...
			if(msg != null) {
				...
				if((msg.hasOwnProperty('completed')) && (msg.completed == 'DONE') && (msg.hasOwnProperty('status')) && ((msg.status == 'OK') || (msg.status == 'ERROR')) && (msg.hasOwnProperty('action')) && (msg.action != null) && (msg.hasOwnProperty('title')) && (msg.title != null) && (msg.hasOwnProperty('message')) && (msg.message != null) && (msg.hasOwnProperty('js_evcode')) && (msg.hasOwnProperty('redirect')) && (msg.hasOwnProperty('replace_div')) && (msg.hasOwnProperty('replace_html'))) {
					...
					if(msg.status == 'OK') { // OK
						...
						if((evcode != undefined) && (evcode != 'undefined') && (evcode != '')) { // undef tests also for null
							try {
								...
								_p$.error(_N$, _m$, 'ERR: JS-Eval Failed (1)', '\nDetails:', err);
							} //end try catch
						} //end if
						//--
						if((msg.js_evcode != undefined) && (msg.js_evcode != '')) { // undef tests also for null
							try {
								eval(evFXStart + String(msg.js_evcode) + evFXEnd); // sandbox
							} catch(err) {
								_p$.error(_N$, _m$, 'ERR: Msg-JS-Eval Failed (1)', '\nDetails:', err);
							} //end try catch
						} //end if
						//--
						...
						if((msg.redirect != null) && (msg.redirect != '') && (msg.message == '')) {
							...
							if((doReplaceDiv != 'yes') || (msg.message != '')) {
								if(haveGrowl === true) {
									MessageNotification(_Utils$.escape_html(String(msg.title)), '<img src="' + _Utils$.escape_html(_C$.param_ImgOK) + '" align="right">' + msg.message, msg.redirect, 'yes', 'green', _C$.param_NotificationTimeOK);
								} else {
									MessageNotification(_Utils$.escape_html(String(msg.action)) + ' / ' + _Utils$.escape_html(msg.title), '<img src="' + _Utils$.escape_html(_C$.param_ImgOK) + '" align="right">' + msg.message, msg.redirect, 'no', '', _C$.param_NotificationTimeOK);
								} //end if else
							} //end if else
							if(haveGrowl !== true) {
								OverlayHide();
						...
						//--
						if((everrcode != undefined) && (everrcode != 'undefined') && (everrcode != '')) { // undef tests also for null
							try {
								if(typeof(everrcode) === 'function') {
									everrcode(the_form_id, url, msg); // call
								} else {
									eval(evFXStart + String(everrcode) + evFXEnd); // sandbox
								} //end if else
							} catch(err) {
								_p$.error(_N$, _m$, 'ERR: JS-Eval Failed (2)', '\nDetails:', err);
							} //end try catch
						} //end if
						//--
						if((msg.js_evcode != undefined) && (msg.js_evcode != '')) { // undef tests also for null
							try {
								...
								_p$.error(_N$, _m$, 'ERR: Msg-JS-Eval Failed (2)', '\nDetails:', err);
							} //end try catch
						} //end if
						//--
						if(haveGrowl === true) {
							MessageNotification('* ' + _Utils$.escape_html(msg.title), '<img src="' + _Utils$.escape_html(_C$.param_ImgNotOK) + '" align="right">' + msg.message, msg.redirect, 'yes', 'red', _C$.param_NotificationTimeERR);
						} else {
							MessageNotification('* ' + _Utils$.escape_html(msg.action) + ' / ' + _Utils$.escape_html(msg.title), '<img src="' + _Utils$.escape_html(_C$.param_ImgNotOK) + '" align="right">' + msg.message, msg.redirect, 'no', '', _C$.param_NotificationTimeERR);
						} //end if else
						if(haveGrowl !== true) {
							OverlayHide();
						} //end if
					...
					//--
					_p$.error(_N$, _m$, 'ERR: Invalid DataObject Format'); // it must not be alert because errors may prevent dialog
					OverlayHide();
					//--
				} //end if else
				//--
				...
				//--
				_p$.error(_N$, _m$, 'ERR: DataObject is NULL'); // it must not be alert because errors may prevent dialog
				OverlayHide();
				//--
			} //end if else
			//--
			...
			//--
			AlertDialog('ERROR: Invalid Server Response !' + '\n' + 'HTTP Status Code: ' + _Utils$.escape_html(msg.status), '', 'ERROR', 750, 425); // + '\n' + msg.responseText
			OverlayHide();
			//--
		});
		//--
	...
	const AjaxRequestByForm = function(the_form_id, url, data_type) { // ES6
		...
		if((url == '') || (url == '#')) {
			_p$.error(_N$, _m$, 'ERR: Invalid URL');
			return null;
		} //end if
		//--
		if(the_form_id == '') {
			...
			if(isMultipart !== true) {
				...
				try {
					...
					ajax = AjaxRequestFromURL(url, 'POST', data_type, data);
					//--
					const warnMsg = 'ERR: ' + _m$ + ' FormData Object Failed. File Attachments were NOT sent ! Try to upgrade / change your browser. It does not support HTML5 File Uploads.';
	 ...
	 */
	const LoadElementContentByAjax = function(y_div, y_img_loader, y_url, y_method, y_data_type, y_data_arr_or_serialized, y_replace=false, y_notifyerr=null) {
		//--
		const _m$ = 'LoadElementContentByAjax';
		//--
		y_div = _Utils$.stringPureVal(y_div, true); // cast to string, trim
		y_div = _Utils$.create_htmid(y_div);
		if(y_div == '') {
		...
		if(y_img_loader != '') {
			if($('#' + y_div + '__' + _Utils$.create_htmid(_m$)).length == 0) {
				$('#' + y_div).prepend('<span id="' + y_div + '__' + _Utils$.create_htmid(_m$) + '"><img src="' + _Utils$.escape_html(y_img_loader) + '" title="Loading ..." alt="Loading ..."></span><br>');
			} //end if
		} //end if
		//--
		...
		ajax.done((msg) => { // {{{JQUERY-AJAX}}}
			...
			if(y_replace === true) {
				$('#' + y_div).replaceWith(divContent);
			} else {
				$('#' + y_div).empty().html(divContent);
			} //end if else
		}).fail((msg) => {
			...
			if(notifyErr === true) {
				AlertDialog('ERROR: Invalid Server Response for LoadDivContent !' + '\n' + 'HTTP Status Code: ' + _Utils$.escape_html(msg.status), '', 'ERROR', 750, 425); // + '\n' + msg.responseText
			} else {
				_p$.error(_N$, _m$, 'ERR: Invalid Server Response', 'HTTP Status Code:', msg.status); // , msg.responseText
			} //end if else
			$('#' + y_div).empty().html(''); // clear
		});
		//--
	}; //END
	 ...
	 */
	const windwScrollDown = (wnd, offset) => { // ES6
		//--
		try {
			wnd.scrollBy(0, _Utils$.format_number_int(parseInt(offset))); // if offset is -1 will go to end
	...
	const WayPoint = function(elSelector, evcode) {
		...
		$(window).scroll(() => {
			...
			if(shouldLoad) {
				scrollTrigger = false;
				//alert('you have scrolled to the target !');
				if((evcode != undefined) && (evcode != '')) { // undef tests also for null
					try {
						if(typeof(evcode) === 'function') {
							evcode($e, elSelector); // call
						} else {
							eval(evFXStart + String(evcode) + evFXEnd); // sandbox
						} //end if else
					} catch(err) {
						_p$.error(_N$, _m$, 'ERR: JS-Eval Failed:', err);
					} //end try catch
				} //end if
			} //end if
		});
	 ...
	 */
	const initModalOrPopUp = function(strUrl, strTarget, windowWidth, windowHeight, forcePopUp, forceDims) {
		//--
		...
		if((((typeof(smartJ$ModalBox) != 'undefined') && (allowModalCascading()) && (forcePopUp != 1)) || (forcePopUp == -1)) && (allowModalBox())) { // use smart modal box
			...
			try { // pre-focus if opened
				if(objRefWinPopup) {
					windowFocus(objRefWinPopup);
				} //end if
			} catch(err){}
			try {
				objRefWinPopup = window.open(strUrl, strTarget, 'top=' + windowTop + ',left=' + windowLeft + ',width=' + windowWidth + ',height=' + windowHeight + ',toolbar=0,scrollbars=1,resizable=1'); // most of modern browsers do no more support display toolbar on popUp
			} catch(err){
				_p$.error(_N$, _m$, 'ERROR raising a new PopUp Window:', err);
			} //end try catch
			if(objRefWinPopup) {
				try { // post-focus
					windowFocus(objRefWinPopup); // focus
				} catch(err){}
				try { // monitor when popup is closed, every 250ms
					let wnd_popup_timer = setInterval(() => {
