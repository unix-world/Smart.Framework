...
const smartJ$Date = new class{constructor(){ // STATIC CLASS
	...
	const getIsoDate = function(date, withTime=false) { // ES6
		...
		//--
		const addLeadingZero = (d_or_m) => { // add leading zero: d_or_m is Integer
			if(d_or_m < 1) {
				d_or_m = 1;
			} //end if
			if(d_or_m < 10) {
		...
		let m = String(addLeadingZero(date.month));
		let d = String(addLeadingZero(date.day));
		//--
		if(withTime === true) {
			return String(y + '-' + m + '-' + d + ' ' + addLeadingZero(date.hour) + ':' + addLeadingZero(date.minute) + ':' + addLeadingZero(date.second));
		} else {
			return String(y + '-' + m + '-' + d);
		} //end if else
		//--
