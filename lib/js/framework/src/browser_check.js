...
const smartJ$TestBrowser = new class{constructor(){ // STATIC CLASS
	...
	const _Option$ = ((typeof(smartJ$Options) != 'undefined') && smartJ$Options && $.isPlainObject(smartJ$Options) && (smartJ$Options.BrowserTest != undefined) && $.isPlainObject(smartJ$Options.BrowserTest)) ? smartJ$Options.BrowserTest : null;
	 ...
	 */
	const checkIsMobileDevice = function() {
		//--
	...
	/**
	 * Detect if a Browser support Cookies or does not have the Cookies disabled.
	...
	/**
	 * Detect if a Browser support ES6 standard for javascript.
	 *
	 * @example
	 * if(!smartJ$TestBrowser.checkJsVersionRequired()) {
	 * 		alert('NOTICE: Your browser does not support Javascript ES6 !');
	 * }
	 *
	 * @memberof smartJ$TestBrowser
	 * @method checkJsRuntime
	 * @static
	 * @arrow
	 *
	 * @returns {Boolean} will return TRUE if Browser supports Cookies and cookies are enabled, FALSE if not or cookies are disabled
	 */
	const checkJsRuntime = () => {
		//--
		try {
			Function('() => {};'); // first have to support arrow type macros, it is a standard for ES6
		} catch(err) {
			return false;
		} //end try catch
		//--
		if(!Number.isSafeInteger) { // the newest ES6 method used on smart framework js is `Number.isSafeInteger` ; {{{SMART-JS-NEWEST-METHOD}}}
			return false;
		} //end if
		//--
		return true;
	...
	_C$.checkJsRuntime = checkJsRuntime; // export
