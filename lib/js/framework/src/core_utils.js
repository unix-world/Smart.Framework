...
const smartJ$Utils = new class{constructor(){ // STATIC CLASS
	 ...
	 * @param 	{String} 	search 			The string to search for in haystack (needle)
	 * @param 	{Boolean} 	caseInsensitive *Optional* ; default is FALSE ; If TRUE will do a case insensitive search
	 * @return 	{Boolean} 					TRUE if the str starts with the search begining from the pos
	 */
	 ...
	 * @return 	{String} 				The processed string
	 */
	const hex2bin = function(hex) { // ES6
		//--
		...
		//--
		hex = String(stringTrim(hex)); // force string and trim to avoid surprises ...
		//--
		let bytes = [], str;
		//--
		...
		//--
		return String(utf8_decode(String.fromCharCode.apply(String, bytes))); // fix to return empty string instead of null
		//--
	}; //END
	...
	const prepareNosyntaxHtmlMarkersTpl = (tpl) => { // ES6
		...
		tpl = String(tpl);
		//--
		tpl = stringReplaceAll('[###', '&lbrack;###', tpl);
		tpl = stringReplaceAll('###]', '###&rbrack;', tpl);
		tpl = stringReplaceAll('[%%%', '&lbrack;%%%', tpl);
		tpl = stringReplaceAll('%%%]', '%%%&rbrack;', tpl);
		tpl = stringReplaceAll('[@@@', '&lbrack;@@@', tpl);
		tpl = stringReplaceAll('@@@]', '@@@&rbrack;', tpl);
	...
	const renderMarkersTpl = function(template, arrobj, isEncoded=false, revertSyntax=false) { // ES6
		//--
		if((typeof(template) === 'string') && (typeof(arrobj) === 'object')) { // syntax: r.20210604
			//--
			if(isEncoded === true) {
										...
										//	} //end if
											...
											if(tmp_marker_arr_esc.length) {
												...
												//	} //end if
													...
													//	} //end if
													//--
													// '|b64' :: !! this is not be exported to JS, it a bit heavyweight for a light templating engine like js ..., and if needed it can build the syntax using concatenation with a b64 encoded string that can be done in js, using smart crypt utils / b64
													//--
													} else {
