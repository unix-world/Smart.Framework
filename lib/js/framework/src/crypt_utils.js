...
window.smartJ$BaseEncode = smartJ$BaseEncode; // export
...
smartJ$CryptoBlowfish = new class{constructor(){ // STATIC CLASS (ES6)
	...
	const setIV = function(key) { // {{{SYNC-CRYPTO-KEY-DERIVE}}}
		...
		let klen = key.length;
		if(klen < 12) { // {{{SYNC-CRYPTO-KEY-MIN}}}
			_p$.warn(_N$, _m$, 'Key Size is lower than 12 bytes (' + klen + ') !');
			return;
		} else if(klen > 1024) { // {{{SYNC-CRYPTO-KEY-MAX}}}
			_p$.warn(_N$, _m$, 'Key Size is higher than 1024 bytes (' + klen + ') !');
			return;
		} //end if
