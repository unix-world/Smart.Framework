...
const smartJ$BaseEncode = new class{constructor(){ // STATIC CLASS (ES6)
	...
	/*
	 * Converts a Base64 (Standard) string to Base64 (Safe URL) by replacing the standard base64 encoding as follows:
	 * '+' with '-',
	 * '/' with '_',
	 * '=' with '.'
	 *
	 ...
	 * @return {String} 					The safe URL Base64 converted string
	 */
	const b64_to_b64s = function(str) {
		str = _Utils$.stringPureVal(str);
		return str.replace(/\+/g, '-').replace(/\//g, '_').replace(/\=/g, '.');
	};
	_C$.b64_to_b64s = b64_to_b64s; // hidden export


	/*
	 * Converts a Base64 (Safe URL) string to Base64 (Standard) by replacing the safe base64 encoding as follows:
	 * '-' with '+',
	 * '_' with '/',
	 * '.' with '='
	 *
	...
	_C$.b64s_to_b64 = b64s_to_b64; // hidden export
	...
	const b64s_enc = function(str, bin=false) {
		//--
		return b64_to_b64s(_Ba$e64.encode(str, bin));
		//--
	}; //END
	...
	const b64s_dec = function(enc, bin=false) {
		//--
		return _Ba$e64.decode(b64s_to_b64(enc), bin);
		//--
	}; //END
