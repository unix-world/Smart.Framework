...
// CONTAINS: Base64 encode/decode ; CryptoHash: CRC32B, MD5, SHA1, SHA512 ; Blowfish (CBC) encrypt / decrypt # r.20210420
...
window.smartJ$TestCrypto = smartJ$TestCrypto; // export
...
window.smartJ$Base64 = smartJ$Base64; // export
 ...
 *
 * @desc Crypto Hash for JavaScript: CRC32B / MD5 / SHA1 / SHA512
 * @author unix-world.org
 * @license BSD
 * @file crypt_utils.js
 * @version 20210420
...
const smartJ$CryptoHash = new class{constructor(){ // STATIC CLASS (ES6)
	...
	let testCryptoHash_CRC32B = false;
	let testCryptoHash_MD5 = false;
	let testCryptoHash_SHA1 = false;
	let testCryptoHash_SHA512 = false;

	const listHexaLower = '0123456789abcdef';
	const listHexaUpper = '0123456789ABCDEF';
	...
	// PRIVATE :: CRYPTO :: the method acts as a setting for hexcase
	const crypt_hexcase = () => {
	...
	const crypt_binl2hex = function(binarray) {
		const tmp_hexcase = 0 + crypt_hexcase();
		const hex_tab = tmp_hexcase ? listHexaUpper : listHexaLower;
		let str = '';
	...
	const crypt_strtoarr = function(string) {
	 ...
	 */
	const crc32b = function(s) {
		//--
		if(testCryptoHash_CRC32B !== true) { // run test
			if(crc32b_hex(_Test$.unicodeString()) == _Test$.testCRC32B()) {
				testCryptoHash_CRC32B = true; // test passed
			} else { // test failed
				_Test$.raiseError('CRC32B/Hash');
				return '';
			} //end if else
		} //end if
		//--
	...
	const md5 = function(s) {
		//--
		if(testCryptoHash_MD5 !== true) { // run test
			if(md5_hex(_Test$.unicodeString()) == _Test$.testMD5()) {
				testCryptoHash_MD5 = true; // test passed
			} else { // test failed
				_Test$.raiseError('MD5/Hash');
				return '';
			} //end if else
		} //end if
		//--
	...
	const sha1 = function(s) {
		//--
		if(testCryptoHash_SHA1 !== true) { // run test
			if(sha1_hex(_Test$.unicodeString()) == _Test$.testSHA1()) {
				testCryptoHash_SHA1 = true; // test passed
			} else { // test failed
				_Test$.raiseError('SHA1/Hash');
				return '';
			} //end if else
		} //end if
		//--
	 ...
	 */
	const sha512 = function(s) {
		//--
		if(testCryptoHash_SHA512 !== true) { // run test
			if(sha512_hex(_Test$.unicodeString()) == _Test$.testSHA512()) {
				testCryptoHash_SHA512 = true; // test passed
			} else { // test failed
				_Test$.raiseError('SHA512/Hash');
				return '';
			} //end if else
		} //end if
		//--
...
/**
 * CLASS :: Smart CryptoBlowfish (ES6)
 *
 * @package Sf.Javascript:Crypto
 *
 * @requires		smartJ$Utils
...
smartJ$CryptoBlowfish = new class{constructor(){ // STATIC CLASS (ES6)
	...
	const setIV = function(key) {
		//-- SmartFramework compatible {{{SYNC-BLOWFISH-IV}}}
		const data = String(_Ba$e64.encode(String(_Crypto$Hash.sha1('@Smart.Framework-Crypto/BlowFish:' + key + '#' + _Crypto$Hash.sha1('BlowFish-iv-SHA1' + key) + '-' + _Crypto$Hash.md5('BlowFish-iv-MD5' + key).toUpperCase() + '#'))).substring(1, (8+1)));
		//_p$.log(_N$, 'setIV', 'iV:', data);
		//-- pre-process
		let ba = array_map(data.split(''), (item) => { return item.charCodeAt(0); });
	...
	const setKey = function(key) {
		...
		const r_key = String(_Crypto$Hash.sha512(key).substring(13, (29+13)) + _Crypto$Hash.sha1(key).substring(13, (10+13)).toUpperCase() + _Crypto$Hash.md5(key).substring(13, (9+13)));
		//_p$.log(_N$, 'setKey', 'Key:', r_key);
		//--
		return r_key;
		//--
	}; //END
	...
	const encrypt = function(plaintext, key) {
		...
		key = setKey(key);
		//--
		const mode = cipher_Modes.CBC;
		const bx = init(key);
		//-- {{{SYNC-BLOWFISH-CHECKSUM}}}
		plaintext = String(plaintext + '#CHECKSUM-SHA1#' + _Crypto$Hash.sha1(plaintext));
		//--
		...
		let cipher = [],
			...
			pos = 0,
			o = {};
		let isCBC = (mode == cipher_Modes.CBC);
		let vector = {
			left: iv.left || null,
			right: iv.right || null
		};
		...
		//=====
		//-- BASE64
		//return _Ba$e64.encode(cipher.join(''));
		//-- HEX
	...
	const decrypt = function(ciphertext, key) {
		//--
		ciphertext = _Utils$.stringPureVal(ciphertext); // cast to string, don't trim ! need to preserve the value
		if(ciphertext == '') {
			return '';
		} //end if
		//--
		...
		//=====
		//-- BASE64
		//c = _Ba$e64.decode(array_map(ciphertext.split(''), function(item){ return item.charCodeAt(0); }));
		//-- HEX
		c = [];
		...
		//=====
		//-- {{{SYNC-BLOWFISH-CHECKSUM}}}
		let parts = plaintext.split('#CHECKSUM-SHA1#');
		plaintext = String(_Utils$.stringTrim(parts[0]));
		const checksum = String(_Utils$.stringTrim(parts[1]));
		parts = null;
		if(_Crypto$Hash.sha1(plaintext) !== (String(checksum))) {
