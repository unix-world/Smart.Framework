...
//(c) 2012-2022 Lea Verou, License: MIT

// (c) 2021-2022 unix-world.org, License: BSD
// r.20221111
// customized by unixman :: #themes=prism-tomorrow&languages=markup+css+clike+javascript+apacheconf+aspnet+bash+basic+batch+bison+c+csharp+cpp+cmake+csp+d+dart+diff+dns-zone-file+go+http+hpkp+hsts+ini+java+json+json5+jsonp+latex+less+log+lua+makefile+markdown+markup-templating+mongodb+nasm+nginx+objectivec+pascal+perl+php+php-extras+properties+python+qml+regex+ruby+rust+sass+scss+shell-session+sql+swift+tcl+toml+typescript+vala+vbnet+vhdl+visual-basic+wasm+yaml&plugins=line-numbers+highlight-keywords
...
// 	- twig templating syntax (light, custom) ; supports extra: django, twist
		...
		util: {
			encode: function encode(tokens) {
				...
				//	return tokens.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/\u00a0/g, ' ');
					return tokens.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/\u00a0/g, ' '); // fix by unixman {{{SYNC-PRISM-ENCODE}}}
				}
			},
...
Prism.languages.nixconf = Prism.languages.apacheconf;
...
Prism.languages.mysqlcfg = Prism.languages.ini;
...
Prism.languages.bindcfg = Prism.languages.nginx;
Prism.languages.dovecotcfg = Prism.languages.nginx;
...
//-- fix: commented out by unixman ; if a heredoc string contains: `\'` will fail to highlight with this enabled
...
//-- #end fix
	...
	//-- fix, by unixman: php-extras, moved here
	...
	Prism.languages.insertBefore('php', 'variable', {
		...
		//-- fix by unixman: disabled 'php_errormsg' as it is deprecated ; added __*__ special variables from PHP
	//	'global': /\$(?:GLOBALS|HTTP_RAW_POST_DATA|_(?:COOKIE|ENV|FILES|GET|POST|REQUEST|SERVER|SESSION)|argc|argv|http_response_header|php_errormsg)\b/,
		'global': /\$(?:GLOBALS|HTTP_RAW_POST_DATA|_(?:COOKIE|ENV|FILES|GET|POST|REQUEST|SERVER|SESSION)|argc|argv|http_response_header)|(__NAMESPACE__|__CLASS__|__METHOD__|__TRAIT__|__FUNCTION__|__DIR__|__FILE__|__LINE__)\b/,
		//-- #end fix
		'scope': {
			pattern: /\b[\w\\]+::/,
			inside: {
...
Prism.languages.pgsqlcfg = Prism.languages.toml;

//==================== unixman

Prism.languages.twig = { // lighter version of twig tpl for prism, r.20210608
	'comment': /\{#[\s\S]*?#\}/,
	'tag': {
		pattern: /\{\{[\s\S]*?\}\}|\{%[\s\S]*?%\}/,
		inside: {
			...
			'keyword': /\b(?:even|if|odd|else|endif|endfor)\b/,
...
Prism.languages.django = Prism.languages.twig;
Prism.languages.twist = Prism.languages.twig; // TODO: make it particulary for Twist ... not use Twig !

//==================== unixman

Prism.languages.markertpl = { // r.20210608
...
(function () {
	...
	Prism.hooks.add('after-highlight', function(env) {
		...
		toolbar.setAttribute('class', 'prism-get-code');

		// View source button
		var linkSource = document.createElement('a');
			linkSource.innerHTML = '&#8904;';
			linkSource.setAttribute('title', 'Get the SourceCode');
			linkSource.setAttribute('rel', 'nofollow'); // fix search engine complaints as link cannot be crawled
		attachClickHandler(linkSource, function() {
			var file = new Blob([this.code], {type: 'text/plain'});
			var fileURL = URL.createObjectURL(file);
			try {
				window.open(fileURL, 'SourceCodePrism', 'resizable,scrollbars,centerscreen,width=800,height=600');
			} catch(e) {
				console.error('Prism SourceCode:', e);
			}
			return false;
		}, env);
	...
	function attachClickHandler(element, callback, that) {
		try {
			...
			alert('FAILED to select source');
			console.error('PrismJs: FAILED to select source', err);
		}
	}
