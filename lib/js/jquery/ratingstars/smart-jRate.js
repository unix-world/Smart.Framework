...
const smartjRate = (ratingArea, ratingRegisterUrl) => {
	...
	if(typeof(ratingArea) == 'undefined') {
		_p$.error(ERRMSG, 'Rating Stars Page is undefined'); // the rating area ; can be the relative URL to the current HTML page ; ex: '/my-page-with-ratings.html' ; or 'sub-dom.mydom.ext/my-page-with-ratings.html' ; MUST NOT CONTAIN ANY URL PREFIX such as: '//' or 'http(s)://'
		return;
	}
	if(typeof(ratingRegisterUrl) == 'undefined') {
		_p$.error(ERRMSG, 'Rating Stars URL is undefined'); // the URL to register the rating ; JSON ; ex: '/handle-ratings.json' or 'http(s)://my-dom.ext/handle-ratings.json' ; {"status":"ERROR|WARN|OK","message":"The Message"}
		return;
	}
	const $ = jQuery;
	...
	if(rUrl == '') {
		_p$.warn(ERRMSG, 'Rating Stars URL is Empty');
		return;
	}
	if(strc$(rUrl, '//', 0)) {
		_p$.warn(ERRMSG, 'Rating Stars URL is Disallowed');
		return;
	}
	...
	$('.rating-stars').each((idx, el) => {
		...
		if(vCnt != '') {
			...
			if(vNum > 0) {
				vTtl = ' ; Votes: ' + html$(vCnt);
				vTxt = '&nbsp;&middot;&nbsp;Votes:&nbsp;' + vNum;
			}
		}
		const id = 'rating-stars-area-' + u$.uuid().toLowerCase();
		const yd = 'jRate-stars-' + u$.uuid().toLowerCase();
		$el.parent().append('<div id="' + html$(id) + '" class="rating-stars-area" title="Rating: ' + html$(rVal) + vTtl + '"><div id="' + html$(yd) + '"></div><span>&nbsp;Rating:&nbsp;<span>' + decf$(rVal, 1, false, true) + '&nbsp;/&nbsp;5' + vTxt + '</span></span></div>');
