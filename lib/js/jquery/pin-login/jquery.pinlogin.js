 ...
 * 		- fix xhtml tags
 * 		- the 1st value after backspace still show the placeholder but value is empty
 */
		...
		$.extend( Plugin.prototype, {
			...
			_attachEvents : function(field, nr) {
				...
				//	if(!$(this).is('[readonly]') && that._values[nr] != undefined && that.settings.hideinput) {
					if(that._values[nr] != undefined && !!that._values[nr].length && that.settings.hideinput) { // fix by unixman #2
						var $fld = $(this);
				...
				//	if((e.keyCode == 37 || e.keyCode == 8) && nr > 0) { // arrow back, backspace
					if((e.keyCode == 37 || e.keyCode == 8) && nr >= 0) { // arrow back, backspace ; fix by unixman #2
						that.resetField(nr);
						if(nr > 0) { // fix by unixman
			...
			resetField : function(nr) {
				this._values[nr] = '';
				if(nr > 0) { // fix by unixman #2
					this._getField(nr)
						.val('')
						.prop('readonly',true)
						.removeClass('invalid');
				} else { // fix by unixman #2
					this._getField(nr) // fix by unixman #2
						.val('')
						.removeClass('invalid');
				}
			},
