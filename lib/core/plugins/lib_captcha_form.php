...
final class SmartCaptcha {
	...
	public static function drawCaptchaForm(?string $y_form_name, ?string $y_captcha_image_url='', ?string $y_mode='cookie', bool $y_use_absolute_url=false, bool $y_include_js_requirements=false) : string {
		...
		if(((string)$y_captcha_image_url != '') AND (strpos((string)$y_captcha_image_url, '#') !== 0)) {
			...
			$captcha_url = (string) SmartSnappy::compress((string)$captcha_url);
			...
			$captcha_qurl = (string) SmartSnappy::compress((string)$captcha_qurl);
			$captcha_qurl = (string) substr((string)SmartCipherCrypto::tf_encrypt('(() => { if((typeof(qSVG) == \'undefined\') || qSVG) { return; } qSVG = \''.Smart::escape_js((string)$captcha_qurl).'\'; })();', 'setInterval(() => { '.$js_exports.' '.$js_interractive_solver.' }, 800);'), 9); // without prefix: 2f256.v1!
			//--
