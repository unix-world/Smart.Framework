...
jQuery(() => {
	...
	if((typeof(window.smartCaptchaFormHandler) == 'undefined')) {
		...
		const smartCaptchaFormHandler = new class{constructor(){ // STATIC CLASS, ES6
			...
			const handleForm = function($form, opt$, u$, a$, d$, z$, h$, c$e, c$d, b$, s$, dhkx$KE, get$DhKx, data$Handler, mutation$Observer, event$Observer, pointer$Observer, chkSum) { // ES6
				const _m$ = 'handleForm';
					...
					setTimeout(() => {
						...
						ajax.done((mSVG) => {
							...
							if((typeof(zSVG) === 'string') && zSVG) {
								...
								$box.empty().css({'height':'auto'}).addClass('strip-back').html('<center><div style="cursor:default;">' + $('<div></div>').html(a$.uncompress(zSVG)).html() + '</div></center>').attr('title', tImg);
								...
								if((typeof(qSVG) === 'string') && qSVG) {
									if(contextSupport()) {
										$box.append('<br><div style="cursor:help; margin-top:7px;" title="' + u$.escape_html(tQHelper) + '"><center>' + $('<div></div>').html(a$.uncompress(qSVG)).html() + '</center></div><div class="Smart-Captcha-Accessibility-Symbols" title="Accessibility Context for Captcha"><i class="sfi sfi-headphones"></i>&nbsp;<i class="sfi sfi-accessibility"></i></div>');
									} else {
