...
if((typeof(smartCaptchaFormHandler) == 'undefined')) {
	...
	const smartCaptchaFormHandler = new class{constructor(){ // STATIC CLASS, ES6
		...
		const handleForm = function($form, $opts, u$, d$, e$, h$, c$, b$, subtle$CryptoDigest, Mutation$Observer, Covariance$Observer, Pointer$Observer, chkSum) { // ES6
			const _m$ = 'handleForm';
			...
			if((chkSum == '') || (chkSum.length != 128)) {
				fxErr('Invalid Captcha Checksum');
				return;
			}
			if(chkSum !== h$.sha512('#' + String($form) + '#' + String($uuid) + '#' + String($opts.imUrl) + '#' + String($opts.iqURL) + '#')) {
			...
			$yc.hide().find('#Smart-Captcha-Icons-Container-' + $uuid).iconsCaptcha({
				clickDelay: u$.format_number_int($delay, false) * 1000,
