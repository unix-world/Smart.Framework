...
jQuery(() => {
	...
	if((typeof(smartCaptchaFormHandler) == 'undefined')) {
		...
		const smartCaptchaFormHandler = new class{constructor(){ // STATIC CLASS, ES6
			...
			const handleForm = function($form, $opts, u$, d$, e$, z$, h$, c$, x$, b$, subtle$CryptoDigest, Mutation$Observer, Covariance$Observer, Pointer$Observer, chkSum) { // ES6
				const _m$ = 'handleForm';
				...
				if(typeof($opts) != 'object') {
					...
					alert('Captcha Options are Undefined !');
					return;
				}

				const $uuid = u$.stringPureVal($opts.uuid || '');
				if(($uuid == '') || ($uuid.length < 10)) {
					_p$.error(_N$, _m$, 'Invalid Captcha UUID');
					alert('Invalid Captcha UUID');
					return;
				}

				const fxErr = (err) => { // must be after UUID validation, it needs $uuid
					err = 'ERR: ' + u$.stringPureVal(err || '', true);
					_p$.error(_N$, _m$, err);
				...
				if($form == '') {
					fxErr('Empty Captcha Form Name');
					return;
				}

				chkSum = u$.stringPureVal(chkSum, true);
				if((chkSum == '') || (chkSum.length < 82) || (chkSum.length > 98)) {
					fxErr('Invalid Captcha Checksum');
					return;
				}
				const calcCkSum = h$.sha512('#' + String($form) + '#' + String($uuid) + '#' + String($opts.imUrl) + '#' + String($opts.iqURL) + '#', true);
				if((chkSum !== smartJ$BaseEncode.b64_to_b64s(calcCkSum)) || (smartJ$BaseEncode.b64s_to_b64(chkSum) !== calcCkSum)) {
					fxErr('Wrong Captcha Checksum');
					return;
				}

				if(typeof($.fn.motionCaptcha) === 'undefined') {
					fxErr('Motion Captcha is N/A');
					return;
				}
				...
				if(!ectx) {
					...
					$('#Smart-Captcha-Input-' + $uuid).empty().html('<div style="text-align:center!important;color:#EF4756!important;"><br><br><b>' + u$.escape_html(canvasErr) + '<b></div>');
					$('#Smart-Captcha-Check-Info-' + $uuid + ' i#Smart-Captcha-Check-Helper').removeClass('sfi-clock2').addClass('sfi-aid-kit').addClass('canvas-error').attr('title', canvasErr);
					return;
				}
				...
				const getDhKx = function(dhkx) {
					...
					return e$.encode(dhkx.shad);
				};
				const evs = (timeout) => 'setInterval(() => { ' + String($opts.exports) + ' ' + 'shiftPointerCovariance(crrPointerPos.x, crrPointerPos.y); $entropy = h$.crc32b($form + \'#\' + subtle$CryptoDigest(JSON.stringify(b$.parseCurrentUrlGetParams()), d$.getIsoDate(new Date(), true))); $dhkx = \'' + getDhKx($opts.dhkx) + '\';' + ' }, ' + timeout + ');';
				...
				const fxTick = () => {
					$('#Smart-Captcha-Area-' + $uuid).find('div#Smart-Captcha-Info-' + $uuid).empty().html('<div style="text-align:center;"><br><img width="24" height="24" src="' + u$.escape_html(String($opts.baseURL) + String($opts.imgTick)) + '"></div>');
				};
						...
						if(timeOut === false) {
							...
							if(!Covariance$Observer(mFy)) {
								setTimeout(() => {
									...
									setTimeout(() => {
										$yc.hide();
										$('#Smart-Captcha-Area-' + $uuid).find('div#Smart-Captcha-Info-' + $uuid).empty().css({'height':'auto'}).html(u$.escape_html(String($opts.tMotion)));
										$mc.show();
									}, 1500);
								}, 750);
								return;
						...
						ajax.done((mSVG) => {
							...
							try {
								...
								if(typeof(mFx) === 'function') {
									...
									_p$.error('ERR:', 'Captcha is not a function');
									return;
								}
							} catch(err){
							...
							if((typeof(zSVG) === 'string') && zSVG) {
								...
								$box.empty().css({'height':'auto'}).addClass('strip-back').html('<center><div style="cursor:default;">' + $('<div></div>').html(e$.decode(zSVG)).html() + '</div></center>').attr('title', tImg);
								...
								if((typeof(qSVG) === 'string') && qSVG) {
									if(contextSupport()) {
										$box.append('<br><div style="cursor:help; margin-top:7px;" title="' + u$.escape_html(tQHelper) + '"><center>' + $('<div></div>').html(e$.decode(qSVG)).html() + '</center></div><div class="Smart-Captcha-Accessibility-Symbols" title="Accessibility Context for Captcha"><i class="sfi sfi-headphones"></i>&nbsp;<i class="sfi sfi-accessibility"></i></div>');
									} else {
				...
				$yc.hide().find('#Smart-Captcha-Icons-Container-' + $uuid).iconsCaptcha({
					...
					iconsURL:   String($opts.baseURL) + String($opts.iIcnsLst + '?u=' + u$.escape_url($uuid) + '&f=' + u$.escape_url($form) + '&e=' + u$.escape_url($entropy) + '&t=' + u$.escape_url(d$.getIsoDate($time, true))),
					loaderImg:  String($opts.baseURL) + String($opts.ldrImg),
					loaderEImg: String($opts.baseURL) + String($opts.ldrEImg),
					hintText:   String($opts.iIcns),
					doneText:   String($opts.iIcnsDone),
				...
				$mc.hide().motionCaptcha({
					...
					maxTries: 7,
					errorMsg: String($opts.tMotionErr),
					warnMsg: String($opts.tMotionWarn),
					completedMsg: String($opts.tMotionDone),
				...
				const eTick = (mouseX, mouseY) => {
					...
					if(!!Pointer$Observer(isHit, arrPointerPos, limPointerPos, arrPointerHit, limPointerHit)) {
						...
						setTimeout(() => {
							$earea.hide();
							$('#Smart-Captcha-Area-' + $uuid).find('div#Smart-Captcha-Info-' + $uuid).empty().css({'height':'auto'}).html(u$.escape_html(String($opts.iIcns)));
							$yc.fadeIn('slow'); // .show()
							setTimeout(() => {
								$yc.trigger('captcha');
							}, 1500);
				...
				$('#Smart-Captcha-Check-Canvas-' + $uuid).on('click', (evt) => { // don't use here: mousedown touchstart ; will trigger later: mouseenter to avoid trigger more than once !
					...
					if(!$obj) {
						_p$.warn('Captcha Tick Invalid Target !');
						return;
					}
